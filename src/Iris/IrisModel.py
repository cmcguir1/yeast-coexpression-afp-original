...
class IrisModel():
    def __init__(self,filePath='./resources/IRIS.csv',network=IrisNet4x3(),lr=0.01,momentum=0.9,bs=4):
        ...
        self.batch = bs
        self.trainloader = torch.utils.data.DataLoader(self.data.trainingData,batch_size=self.batch,shuffle=True,num_workers=2)
        self.testloader = torch.utils.data.DataLoader(self.data.testingData,batch_size=self.batch,shuffle=True,num_workers=2)
        ...
        #Intialize loss function and optimizer, along with optimizer hyperparameters
        self.lossFunc = torch.nn.CrossEntropyLoss()
        self.learningRate = lr
        self.momentum = momentum
        self.opt = optim.SGD(self.net.parameters(),lr=self.learningRate,momentum=self.momentum)

    #Resets network with new randomized data and an untrained newtwork
    def resetModel(self):
        self.data = IrisData(0.7,filePath=self.dataPath)
        self.trainloader = torch.utils.data.DataLoader(self.data.trainingData,batch_size=self.batch,shuffle=True,num_workers=2)
        self.testloader = torch.utils.data.DataLoader(self.data.testingData,batch_size=self.batch,shuffle=True,num_workers=2)
        #This method should reset all of the weights and biases of each layer of the net, but I might go a different route later
        for para in self.net.children():
            if(hasattr(para,'reset_parameters')):
                para.reset_parameters()
        #Reset the optimizer
        self.opt = optim.SGD(self.net.parameters(),lr=self.learningRate,momentum=self.momentum)
    ...
    def testNetworkAccuracyTraining(self):
        return self.testNetworkAccuracy(self.trainloader,'Training Data')

    #Tests the model's accuracy on data it has not seen before
    def testNetworkAccuracyTesting(self):
        return self.testNetworkAccuracy(self.testloader, 'Testing Data')
