...
def main():
    ...
    for term in commonTerms - set(['GO:0006360']):
        ...
        while len(ogPairs) < subsetSize:
            ...
            if geneA != geneB:
                ogPairs.append(corrDict.lookupAverageCorrelation(geneA,geneB))
        ...
        geneData = [[ogPairs[i],modPairs[i]] for i in range(subsetSize)]
        pd.DataFrame(geneData,columns=['2007','2022']).to_csv(f'./Yeast Resources/CorrelationData/{term[:2]}{term[3:]}_Pairs.csv',index=False)
