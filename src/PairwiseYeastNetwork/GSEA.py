...
def GSEA(df,scoreCol,p=1,termCutoff=10,label='',name='GSEA'):
    ...
    pd.DataFrame(table,columns=['GO Term ID','GO Term Name','GO Term Size','Num Annotations','ES','p-value',f'NES','FDR q-value','LES size','Leading-edge subset']).to_csv(f'./Yeast Resources/GSEA_q/{name}_{scoreCol}{"_"+label[0]+ "_" + label[2] if label != "" else ""}.csv',index=False)
