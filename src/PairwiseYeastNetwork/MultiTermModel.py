...
class MultiTermModel():
    def __init__(self,fold,structure,folderName,modelName,numFolds=4,lr=0.01,min_lr=1e-7,momentum=0.9,batch=50,gamma=2,alpha=1,weightDecay=0.01,weighted=False,lossFunc='CE',foldFile='./src/PairwiseYeastNetwork/AllGOGeneFold1.csv',ontologyDataset='modern',regularize=False,inputDropout=None,hiddenDropout=None,activation='relu',resetNet=False,cuda=True,inputVector = 'x',outputVector = 'b',addTerms=[],memMapName='YeastDict_Regularized.npy',randomizeLabels=False,randomizeFeatures=False,swapGenes=False):
        ...
        weightDecay_Name = '' if weightDecay == 0.01 else f'_wd{weightDecay}'
    ...
    def trainNetwork(self,epochs,track=100,printTensors=False,numTest=10):
        ...
        posPairs, _ = self.makePosNegPairs(self.training)
        testPairs, _ = self.makePosNegPairs(self.validation)
