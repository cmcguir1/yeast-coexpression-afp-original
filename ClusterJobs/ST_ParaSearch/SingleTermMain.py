...
def main():
    ...
    if sys.argv[4] == 'runAll':
        for i in range(4):
            model = PairwiseModel(modelData,i,f'{sys.argv[2]}x1','Spell/lr_struct_ParaSearch',f'{term[0:2]}{term[3:]}',lr=float(sys.argv[3]),resetNet=True)
        ...
        model = PairwiseModel(modelData,int(sys.argv[4]),f'{sys.argv[2]}x1','Spell/SingleTerm_lr_0.01',f'{term[0:2]}{term[3:]}',batch=50,lr=0.01,resetNet=True)
