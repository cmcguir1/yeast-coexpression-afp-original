...
def main():
    ...
    GOTest = AllGoModel(int(sys.argv[1]),'25000','CE_OnlyPos',f'Original',foldFile='./src/PairwiseYeastNetwork/AllGOGeneFold1.csv',ontologyDataset='original',resetNet=True,inputVector='x',outputVector='b',cuda=False,lossFunc='CE',lr=float(sys.argv[2]))
    GOTest.trainNetwork(100000,track=100,onlyPos=True,partition=False)
    GOTest.testNetworkAll(runAll=True)
    GOTest.testNetworkAll(runAll=True,validation=False)
