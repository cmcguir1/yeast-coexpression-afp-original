...
def main():
    ...
    GOTest = AllGoModel(int(sys.argv[1]),'25000','CE_OnlyPos_ps',f'Original',foldFile='./src/PairwiseYeastNetwork/AllGOGeneFold1.csv',ontologyDataset='original',resetNet=True,inputVector='x',outputVector='b',cuda=False,lossFunc=sys.argv[3],lr=float(sys.argv[2]))
    GOTest.trainNetwork(250000,track=100,onlyPos=True,partition=False)
    GOTest.testNetworkAll(runAll=True)
    GOTest.testNetworkAll(runAll=True,validation=False)
