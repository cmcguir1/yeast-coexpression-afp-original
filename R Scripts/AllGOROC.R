...
plotROC <- function(files,colorsList,graphName) {
  ...
  data <- data[order(data[,"Confidence"],decreasing = FALSE),]
  #Creates a plot of the first data series
  plot(data[,"falsePositiveRate"], data[,"recall"],xlim=c(0,1), ylim= c(0,1), type="l", col=colorsList[1],
  ...
  for (i in 2:length(files)) {
    ...
    lines(data[,"falsePositiveRate"],data[,"recall"],lwd=width,col=colorsList[i])
